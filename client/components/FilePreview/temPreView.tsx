...
const ProviewView = ({ preViewFile, classes, setPreViewFile }) => {
  let applicationForm = '';
  applicationForm = `/api/file/template/preview?tempDataKey=${preViewFile.resourceId}`;
  const resourceName = preViewFile && preViewFile.resourceName;
  const index = resourceName.lastIndexOf('.');
  const fileType = resourceName.substr(index + 1);
  if (fileType === 'pdf') {
    applicationForm = `/api/file/preview/${preViewFile.resourceId}/${resourceName}`;
  } else {
    applicationForm = `/api/file/template/preview?tempDataKey=${preViewFile.resourceId}`;
  }
